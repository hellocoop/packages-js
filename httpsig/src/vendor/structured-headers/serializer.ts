 ...
 * directory. Upstream: src/serializer.ts
 */
...
export function serializeInteger(input: number): string {
    if (input < -999_999_999_999_999 || input > 999_999_999_999_999) {
        throw new SerializeError(
            'Structured headers can only encode integers in the range range of -999,999,999,999,999 to 999,999,999,999,999 inclusive',
        )
    }
...
export function serializeDecimal(input: number): string {
    ...
    if (signifantDigits > 12) {
        throw new SerializeError(
            'Fractional numbers are not allowed to have more than 12 significant digits before the decimal point',
        )
    }
    return out
}
...
export function serializeKey(input: string): string {
    if (!isValidKeyStr(input)) {
        throw new SerializeError(
            'Keys in dictionaries must only contain lowercase letter, numbers, _-*. and must start with a letter or *',
        )
    }
    return input
}
