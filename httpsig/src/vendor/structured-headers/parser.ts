 ...
 * directory. Upstream: src/parser.ts
 */
...
export default class Parser {
        ...
        while (!this.eof()) {
            ...
            if (this.eof()) {
                throw new ParseError(
                    this.pos,
                    'Dictionary contained a trailing comma',
                )
            }
        }
        return dictionary
    ...
    parseItem(standaloneItem: boolean = true): Item {
        ...
        const result: Item = [this.parseBareItem(), this.parseParameters()]
    ...
    private parseInnerList(): InnerList {
        ...
        while (!this.eof()) {
            ...
            if (nextChar !== ' ' && nextChar !== ')') {
                throw new ParseError(
                    this.pos,
                    'Expected a whitespace or ) after every item in an inner list',
                )
            }
        }
    ...
    private parseIntegerOrDecimal(): number {
        ...
        while (!this.eof()) {
            ...
            if (isDigit(char)) {
                ...
                if (inputNumber.length > 12) {
                    throw new ParseError(
                        this.pos,
                        'Exceeded maximum decimal length',
                    )
                }
                inputNumber += '.'
                type = 'decimal'
            ...
            if (type === 'integer' && inputNumber.length > 15) {
                throw new ParseError(
                    this.pos,
                    'Exceeded maximum integer length',
                )
            }
        ...
        if (type === 'integer') {
            ...
            if (inputNumber.split('.')[1].length > 3) {
                throw new ParseError(
                    this.pos,
                    'Number of digits after the decimal point cannot exceed 3',
                )
            }
        ...
        while (!this.eof()) {
            ...
            if (char === '\\') {
                ...
                if (nextChar !== '\\' && nextChar !== '"') {
                    throw new ParseError(
                        this.pos,
                        'A backslash must be followed by another backslash or double quote',
                    )
                }
                ...
                throw new ParseError(
                    this.pos,
                    'Strings must be in the ASCII range',
                )
            } else {
                outputString += char
            }
    ...
    private parseByteSequence(): ByteSequence {
        ...
        if (endPos === -1) {
            throw new ParseError(
                this.pos,
                'Could not find a closing ":" character to mark end of Byte Sequence',
            )
        }
        ...
        if (!/^[A-Za-z0-9+/=]*$/.test(b64Content)) {
            throw new ParseError(
                this.pos,
                'ByteSequence does not contain a valid base64 string',
            )
        }
    ...
    private parseBoolean(): boolean {
        ...
        throw new ParseError(
            this.pos,
            'Unexpected character. Expected a "1" or a "0"',
        )
    }
    ...
    private parseKey(): string {
        if (!this.lookChar()?.match(/^[a-z*]/)) {
            throw new ParseError(
                this.pos,
                'A key must begin with an asterisk or letter (a-z)',
            )
        }
