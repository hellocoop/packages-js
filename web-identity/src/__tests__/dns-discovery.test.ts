...
describe('DNS Discovery Functions', () => {
    ...
    describe('discoverIssuer', () => {
        it('should discover issuer from email address', async () => {
            ...
            expect(mockDns).toHaveBeenCalledWith(
                'email._web-identity.example.com',
            )
        })
    ...
    describe('fetchWebIdentityMetadata', () => {
        const mockMetadata = {
            issuance_endpoint:
                'https://accounts.issuer.example/web-identity/issuance',
        ...
        it('should fetch metadata successfully', async () => {
            mockFetch.mockResolvedValue({
                ok: true,
                url: 'https://issuer.example/.well-known/web-identity',
                headers: new Map([['content-type', 'application/json']]),
                json: () => Promise.resolve(mockMetadata),
            })

            const result = await fetchWebIdentityMetadata('issuer.example')

            expect(result).toEqual(mockMetadata)
            expect(mockFetch).toHaveBeenCalledWith(
                'https://issuer.example/.well-known/web-identity',
        ...
        it('should handle redirects to subdomains', async () => {
            mockFetch.mockResolvedValue({
                ok: true,
                url: 'https://accounts.issuer.example/.well-known/web-identity',
                headers: new Map([['content-type', 'application/json']]),
                json: () => Promise.resolve(mockMetadata),
            })
        ...
        it('should use cache on subsequent calls', async () => {
            ...
            // First call
            await fetchWebIdentityMetadata('issuer.example')
        ...
        it('should throw error for HTTP error responses', async () => {
            ...
            await expect(
                fetchWebIdentityMetadata('issuer.example'),
        ...
        it('should throw error for invalid redirect domain', async () => {
            mockFetch.mockResolvedValue({
                ok: true,
                url: 'https://evil.com/.well-known/web-identity',
                headers: new Map([['content-type', 'application/json']]),
                json: () => Promise.resolve(mockMetadata),
            })
        ...
        it('should handle timeout', async () => {
            ...
            await expect(
                fetchWebIdentityMetadata('issuer.example', { timeout: 100 }),
            ).rejects.toThrow(JWKSFetchError)
        })
    })
