...
import { WebIdentityError } from '../errors.js'
 ...
 * Looks for TXT record with format: "iss=issuer.example" at "email._web-identity.$EMAIL_DOMAIN"
 *
...
export async function discoverIssuer(emailOrDomain: string): Promise<string> {
    ...
    // Construct the DNS lookup domain per spec: email._web-identity.$EMAIL_DOMAIN
    const lookupDomain = `email._web-identity.${normalizedDomain}`
...
/**
 * Web Identity metadata structure from /.well-known/web-identity
 */
...
const metadataCache = new Map<
    string,
    { metadata: WebIdentityMetadata; timestamp: number }
>()
...
/**
 * Fetches web-identity metadata from an issuer domain
 * Follows the spec path: https://issuer.example/.well-known/web-identity
 ...
 * @returns Promise resolving to web-identity metadata
...
export async function fetchWebIdentityMetadata(
    ...
    try {
        ...
        const metadataUrl = `https://${normalizedIssuer}/.well-known/web-identity`
        ...
        if (!response.ok) {
            throw new JWKSFetchError(
                `Web-identity metadata fetch failed: ${response.status} ${response.statusText}`,
            )
        }
        ...
        const metadata = (await response.json()) as WebIdentityMetadata
 ...
 *
 * @param jwksUri - JWKS URI from web-identity metadata
 * @param options - Optional request configuration
