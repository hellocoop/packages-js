...
const convertToHelloRequest = (
    ...
    return {
        headers: () => {
            return req.headers as { [key: string]: string }
        },
...
const convertToHelloResponse = (res: FastifyReply): HelloResponse => {
    return {
        ...
        getHeaders: () => {
            return res.getHeaders() as Record<string, string>
        },
...
const helloPlugin: FastifyPluginAsync<HelloConfig> = async (
    instance,
    options,
) => {
    if (!isConfigured) configure(options)
