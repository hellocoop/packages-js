...
const handleInvite = async (req: HelloRequest, res: HelloResponse) => {
    ...
    if (!auth.sub)
        return res.status(401).send('Missing sub in auth')
    if (!auth.email)
        return res.status(401).send('Missing email in auth')
    if (!auth.name)
        return res.status(401).send('Missing name in auth')
    if (!app_name)
        return res.status(400).send('Missing app_name')
