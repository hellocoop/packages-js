...
const _configuration: IConfig = {
    ...
    secret:
        ...
        (process.env.HELLO_COOKIE_SECRET as string),
    host: HOST,
    // for internal testing
...
export const configure = function (config: Config) {
    ...
    const apiRoute =
        process.env.HELLO_API_ROUTE || config.apiRoute || '/api/hellocoop'
    ...
    else _configuration.provider_hint = config.provider_hint

    if (process.env.HELLO_DEBUG) _configuration.logDebug = true

    _configuration.sameSiteStrict =
    ...
    if (!_configuration.clientId) {
        const message =
            'No CLIENT_ID or HELLO_CLIENT_ID was in environment or client_id in hello.config'
    ...
    if (!_configuration.secret) {
        const message =
            'No COOKIE_SECRET or HELLO_COOKIE_SECRET was in environment'
        _configuration.error = [message]
        console.error(message)
        isConfigured = false
    }
    ...
    while (pendingConfigurations.length > 0) {
        ...
        if (resolve) resolve(_configuration)
    }
    if (config.logConfig)
        console.log(
...
export const getLoginApiRoute = (): string => {
    return _configuration.loginApiRoute
}
export const getLogoutApiRoute = (): string => {
    return _configuration.logoutApiRoute
}
export const getAuthApiRoute = (): string => {
    return _configuration.authApiRoute
}
